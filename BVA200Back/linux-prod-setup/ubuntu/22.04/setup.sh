...
apt update && apt upgrade -y && apt install -y flatpak firefox dotnet-sdk-6.0 dotnet-runtime-6.0 printer-driver-all gconf2 wkhtmltopdf p11-kit apt-transport-https nodejs npm xdotool libusb-1.0-0 ca-certificates openssl libnss3-tools espeak
...
su -c '
    ...
    Name=BVA200
    Comment=
    Exec=$HOME/Desktop/publish/linux-prod-setup/ubuntu/22.04/init.sh
    Terminal=false
